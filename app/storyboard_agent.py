...
DESCRIPTION = (
    ...
    "scene storyboards based on a screenplay and character descriptions."
)


def _load_gcs_image_part(
    uri: str, project_id: str
) -> types.Part | None:
    """Helper to load an image from a GCS URI as a genai Part."""
    if not uri:
        return None
    ...
    try:
        ...
        logger.warning(f"Could not load reference image from '{uri}': {e}")
...
def create_character_profile(
    ...
    """Create a Master Character Reference Sheet from a character description.

    Ensures the character's facial structure, hair, clothing, and palette remain
    consistent across all scene storyboards and videos without character drift.

    Args:
        character_name (str): Name of the character (e.g., 'Leo', 'Sammy').
        visual_description (str): Detailed visual traits, clothing, colors, and distinct features.
        visual_style (str): The unified art style (e.g., 'Cinematic 3D Animation').
    ...
    try:
        ...
        logger.info(
            f"Creating master character reference profile for '{character_name}'"
        )
...
def storyboard_generate(
    ...
    try:
        ...
        for char_name, uri in char_ref_dict.items():
            part = _load_gcs_image_part(uri, project_id)
            if part:
